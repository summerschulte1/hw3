...
void llpivotRecursive(Node* current, Node*& smaller, Node*& larger, int pivot);
...
Node* llfilter(Node* head, Comp pred)
{
    ...
    if (pred(head->val)) {
        ...
        // If the current node should be kept, recursively call llfilter on the rest of the list
